...
# val_opt.num_objects = [2] # Only matters for MNIST TODO: this has been modified
...
for epoch in range(opt.start_epoch, opt.n_epochs):
  ...
  for step, data in enumerate(train_loader):
    # Note: Currently, output = man. When we predict (if we do) output will correspond to future frames
    input, output, neigh, dists, man = data
    _, loss_dict = model.train(*data[:-1])

    if step % opt.log_every == 0:
      # Write to tensorboard
      vis.add_scalar(loss_dict, epoch * len(train_loader) + step)
      # Visualization
      model.test(input, output, neigh, dists, epoch=epoch, save_every=save_every)
