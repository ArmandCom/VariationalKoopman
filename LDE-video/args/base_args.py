...
class BaseArgs:
  ...
  def __init__(self):
    ...
    self.parser.add_argument('--gpus', type=str, default='0,1', help='visible GPU ids, separated by comma')

    # data
    self.parser.add_argument('--dset_dir', type=str, default=os.path.join('/data/BDSC/', 'datasets/'))
    self.parser.add_argument('--dset_name', type=str, default='small_norb')#Synthetic_Mocap
    ...
    self.parser.add_argument('--time_enc_size', type=int, default=64,
                             help='Size of temporal encoding')
    self.parser.add_argument('--t_enc_rnn_hidden_size', type=int, default=128,
                             help='Size of the hidden size of the time enc rnn')
    self.parser.add_argument('--t_enc_rnn_output_size', type=int, default=128,
                             help='Size of the output size of the time enc rnn')
    self.parser.add_argument('--trans_rnn_output_size', type=int, default=128,
                             help='Size of the output size of the transition rnn')
    self.parser.add_argument('--manifold_size', type=int, default=256,
                             help='Dimension of the manifold for the given time sequence')#64

    # Changing hyperparameters
    self.parser.add_argument('--n_frames_input', type=int, default=9)
    self.parser.add_argument('--n_frames_output', type=int, default=0)

    self.parser.add_argument('--weight_dim', type=float, default=0,
                             help='Weight of the manifold dimension loss - alpha')#0.1
    self.parser.add_argument('--weight_local_geom', type=float, default=0,
                             help='Weight of local geometry loss - beta')#10000
    self.parser.add_argument('--weight_lin', type=float, default=0,
                             help='Weight of the linearity loss - gamma') #7
    self.parser.add_argument('--weight_rec', type=float, default=1,
                             help='Weight of the reconstruction loss - pie')
    ...
    self.parser.add_argument('--ckpt_name', type=str, default='test1', help='checkpoint name')
    ...
    self.parser.add_argument('--evaluate_every', type=int, default=4, help='evaluate on val set every x epochs')
  ...
  def parse(self):
    ...
    if opt.dset_name == 'moving_mnist':
        ...
        opt.n_channels = 1
        opt.image_size = (96, 96)
    else:
        # Hard code
        opt.n_channels = 1
        opt.image_size = (1, 63*3)
