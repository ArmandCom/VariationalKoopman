...
class PoseRNN(nn.Module):
  ...
  def encode(self, input):
    ...
    for i in range(self.n_components): # Note: In case of mnist n_comp = 2
      ...
      # Note: frame_outputs is the concatenation of frames after encoded+RNN
      encoder_outputs.append(frame_outputs)

      # Save last hidden states (h, c)

      # Note: there will be a pair per component
      # TODO: print hidden_states length and size of components to check
    ...
    # Note: beta_mu_layer and ""sigma"" have input_size = 64, output_size = 3 --> output: [N*T*ncomp,3]
    ...
    initial_mu, initial_sigma = self.get_initial_pose(first_hidden_states) #TODO: analize this function

    #TODO: check what they do with the two encoded components to make them dependent of each other (possibly not in this function)
